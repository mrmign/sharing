    ...
    def get(self,user_id): 
        ...
        if self.current_user:
            if u.id==self.current_user.id:
                self.render("profile.html" ,user=self.current_user ,followers=followers,followings=followings,groups=self.current_user.groups,description=self.current_user.introduction)
            else:
